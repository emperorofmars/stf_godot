...
## Exports the given root-node into the STF_File class, which can write to disk.
...
static func export(path: String, root: SceneTree):
	...
	var export_state = STF_ExportState.new(STF_Registry.get_handlers_by_godot_type(), root.edited_scene_root.get_meta("stf", {}).get("import_meta"))
