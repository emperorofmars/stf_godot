class_name STFEXP_Constraint_Parent
extends STF_Handler

func _get_stf_type() -> String: return "stfexp.constraint.parent"
...
func __create_finalize_source_func(ret: CopyTransformModifier3D, bone_index: int, stf_id: String, json_resource: Dictionary, constraint_indices: Array, axes) -> Callable:
	return func(ref_type: int, reference: Variant, handle_context: Variant):
		...
		ret.set_copy_flags(constraint_index, CopyTransformModifier3D.TRANSFORM_FLAG_ALL)
...
func _import(context: STF_ImportContext, stf_id: String, json_resource: Dictionary, context_object: Variant, instance_context: Variant) -> ImportResult:
	...
	ret.name = STF_Godot_Util.get_name_or_default(json_resource, "STF Constraint Parent - " + armature.get_bone_name(bone_index))
	...
	var convert_axes := func(json_key: String) -> int:
		var json_axes: Array = json_resource.get(json_key, [true, true, true])
		var axes = 0
	...
	var axes_rotation = convert_axes.call("rotation_axes")
	var axes_translation = convert_axes.call("translation_axes")
	if(axes_rotation != axes_translation):
		print_rich("[color=orange]Warning: Can't import resource [u]" + _get_stf_type() + "[/u] with ID [u]" + stf_id + "[/u][/color]: Godot constraints can't represent different rotation and translation axes.")
		return null
	...
	var finalize_source_func := __create_finalize_source_func(ret, bone_index, stf_id, json_resource, constraint_indices, axes_rotation)
